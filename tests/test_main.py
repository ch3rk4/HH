...
def test_search_vacancies_success():
    ...
    with patch("main.HH", return_value=mock_hh), patch(
        "main.JSONSaver", return_value=mock_json_saver
    ), patch("main.Vacancy.cast_to_object_list", return_value=mock_vacancy_list), patch(
        "main.sort_vacancies", return_value=mock_vacancy_list
    ), patch(
        "main.get_top_vacancies", return_value=mock_vacancy_list
    ), patch(
        "main.print_vacancies"
    ), patch(
        "builtins.input", side_effect=["1", "Python", "да", "2"]
    ):
...
def test_get_top_vacancies():
    ...
    with patch("main.HH", return_value=mock_hh), patch(
        "main.JSONSaver", return_value=mock_json_saver
    ), patch("main.sort_vacancies", return_value=mock_vacancy_list), patch(
        "main.get_top_vacancies", return_value=mock_vacancy_list
    ), patch(
        "main.print_vacancies"
    ) as mock_print, patch(
        "builtins.input", side_effect=["2", "5"]
    ):
        # Запускаем функцию
        main.user_interaction(test_mode=True)
...
def test_filter_vacancies():
    ...
    with patch("main.HH", return_value=mock_hh), patch(
        ...
        "main.filter_vacancies", return_value=mock_vacancy_list
    ) as mock_filter, patch(
        "main.get_vacancies_by_salary", return_value=mock_vacancy_list
    ) as mock_salary, patch(
        "main.print_vacancies"
    ) as mock_print, patch(
        "builtins.input", side_effect=["3", "Python Django", "100000 - 200000"]
    ):
        # Запускаем функцию
        main.user_interaction(test_mode=True)
...
def test_invalid_choice():
    ...
    # Патчим зависимости
    with patch("main.HH"), patch("main.JSONSaver"), patch(
        "builtins.print"
    ) as mock_print, patch("builtins.input", return_value="10"):
        # Запускаем функцию
        main.user_interaction(test_mode=True)
...
def test_empty_vacancies():
    ...
    with patch("main.HH", return_value=mock_hh), patch(
        "main.JSONSaver", return_value=mock_json_saver
    ), patch("builtins.print") as mock_print, patch("builtins.input", return_value="2"):
        # Запускаем функцию
        main.user_interaction(test_mode=True)

        # Проверяем, что было выведено сообщение об отсутствии вакансий
        mock_print.assert_any_call(
            "В файле нет сохраненных вакансий. Сначала выполните поиск."
        )
...
def test_api_error():
    ...
    with patch("main.HH", return_value=mock_hh), patch(
        ...
        "builtins.input", side_effect=["1", "Python"]
    ):
        # Запускаем функцию
        main.user_interaction(test_mode=True)
